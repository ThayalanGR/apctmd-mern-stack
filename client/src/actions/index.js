  ...
  return async dispatch => {
    try {
      const res = await axios.post(
        "https://apctmd.herokuapp.com/api/signup",
        data
      );
...
export const signIn = data => {
  return async dispatch => {
    try {
      const res = await axios.post(
        "https://apctmd.herokuapp.com/api/signin",
        data
      );
      axios.defaults.headers.common["Authorization"] = `Token ${
        res.data.user.token
...
export const getSecret = () => {
  return async dispatch => {
    try {
      const res = await axios.get("https://apctmd.herokuapp.com/api/current");
      dispatch({
...
export const getSensorData = () => {
  return async dispatch => {
    try {
      const res = await axios.get("https://apctmd.herokuapp.com/api/gasout");
      dispatch({
        type: GAS_SENSOR,
